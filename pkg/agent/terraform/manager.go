...
type Manager interface {
	RunPlan(ctx context.Context, workspaceDir, stackName, toolVersion, planFlags string, output io.Writer) (string, error)
	RunApply(ctx context.Context, workspaceDir, stackName, toolVersion string, output io.Writer) (string, error)
}
...
func (m *manager) RunPlan(ctx context.Context, workspaceDir, stackName, toolVersion, planFlags string, output io.Writer) (string, error) {
	...
	if err := m.runner.Init(ctx, terraformBin, terraformDir, output); err != nil {
		return "", err
	}

	return m.runner.Plan(ctx, terraformBin, terraformDir, planFlags, output)
}

func (m *manager) RunApply(ctx context.Context, workspaceDir, stackName, toolVersion string, output io.Writer) (string, error) {
	terraformDir, version, err := m.resolveStack(workspaceDir, stackName, toolVersion)
	if err != nil {
		return "", err
	}
	...
	return m.runner.Apply(ctx, terraformBin, terraformDir, output)
}
