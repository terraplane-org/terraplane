...
| orchestrator.replicaCount | int | `1` | Number of orchestrator replicas (HA-safe with HTTP job claim) |
...
| agentDefaults.orchestratorURL | string | `""` | HTTP base URL for agents (required when agents is non-empty). Example: `https://terraplane.example.com` |
